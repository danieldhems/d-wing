...
export default class UpperTurret extends Ship {
	constructor(options){
		super(options)
		Object.assign(this, options);
		this.type = 'Upper Turret';
	...
	getPosition(){
		if(this.deploymentComplete){
			return {
				x: this.source.getBoundingBox().left,
				y: this.source.getBoundingBox().top - this.source.turretVerticalPositionOffset - this.turrets.height
			}
		} else {
			this.deploymentProgress += this.turrets.deploymentRate;
			if(this.deploymentProgress >= this.source.turretVerticalPositionOffset + this.turrets.height) this.deploymentComplete = true;
			return {
				x: this.source.getBoundingBox().left,
	...
	draw(){
		...
		if(Date.now() - this._lastSpriteFrame > this.source.weaponConfig.turrets.animRate){
			this._lastSpriteFrame = Date.now();
